 ...
 */
BlockuinoPixelFigure3x3::BlockuinoPixelFigure3x3(const String colorsStr[9], int startIndex, int pSize)
{
    panelSize = pSize;
    figureSize = 3;
    position = startIndex;
...
/**
 * Draw the 3x3 figure onto the NeoPixel strip.
 */
void BlockuinoPixelFigure3x3::draw() {
    ...
    // Iterate through the 3x3 figure
 ...
 */
bool BlockuinoPixelFigure3x3::isPixelOccupied(int pixelIndex) const {
    for (int r = 0; r < figureSize; r++) {
        for (int c = 0; c < figureSize; c++) {
            int rotatedRow, rotatedCol;
