 ...
 */
BlockuinoPixelFigure2x2::BlockuinoPixelFigure2x2(const String colorsStr[4], int startIndex, int pSize)
{
    panelSize = pSize;
    figureSize = 2;
    position = startIndex;
 ...
 */
void BlockuinoPixelFigure2x2::draw() {
    ...
    // Iterate through the 2x2 figure
 ...
 */
bool BlockuinoPixelFigure2x2::isPixelOccupied(int pixelIndex) const {
    for (int r = 0; r < figureSize; r++) {
        for (int c = 0; c < figureSize; c++) {
            int rotatedRow, rotatedCol;
